...
export function RegisterForm() {
  ...
  const inputClass = (field: string) =>
    `w-full rounded-xl border bg-white px-4 py-3 text-sm outline-none transition-all ${fieldErrors[field] ? 'border-red-400 focus:ring-2 focus:ring-red-200' : 'border-gray-200 focus:border-orange-400 focus:ring-2 focus:ring-orange-100'}`;
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select value={countryCode} onChange={(e) => { setCountryCode(e.target.value); setForm({ ...form, phone: '' }); }} className="w-28 rounded-xl border border-gray-200 bg-white px-2 py-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
            ...
            {form.password.length > 0 && (
              <div className="mt-2 flex gap-1">
                {[1, 2, 3, 4].map((i) => {
                  ...
                  return <div key={i} className={`h-1 flex-1 rounded-full ${i <= s ? c[s - 1] : 'bg-gray-200'}`} />;
                })}
              </div>
            )}
          </div>
          ...
          <Button
            type="submit"
            className={`w-full rounded-xl py-6 text-base font-semibold text-white shadow-lg ${isOrganiser ? 'bg-gray-900 hover:bg-gray-800 shadow-gray-900/20' : 'bg-orange-500 hover:bg-orange-600 shadow-orange-500/20'}`}
