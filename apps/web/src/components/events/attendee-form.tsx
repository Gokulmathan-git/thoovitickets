    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Attendee Information</h2>
            <p className="text-sm text-gray-500">{filledCount}/{totalTickets} tickets filled</p>
          </div>
            ...
            return (
              <div
                ...
                className={`rounded-xl border-2 transition-all ${
                  isExpanded ? 'border-orange-400 bg-orange-50/30' : isFilled ? 'border-green-300 bg-green-50/30' : 'border-gray-200'
                }`}
              >
                {/* Ticket Header */}
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      {isFilled && !isExpanded && (
                        <p className="text-xs text-gray-500">{attendee.name} · {attendee.email}</p>
                      )}
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 p-4 space-y-3">
                    <div>
                      ...
                      <input
                        className={`w-full rounded-lg border px-3 py-2.5 text-sm outline-none transition ${
                          errors[`${index}-name`] ? 'border-red-400' : 'border-gray-200 focus:border-orange-400'
                        }`}
                    ...
                    <div>
                      ...
                      <input
                        className={`w-full rounded-lg border px-3 py-2.5 text-sm outline-none transition ${
                          errors[`${index}-email`] ? 'border-red-400' : 'border-gray-200 focus:border-orange-400'
                        }`}
                        type="email"
                    ...
                    <div>
                      ...
                      <input
                        className={`w-full rounded-lg border px-3 py-2.5 text-sm outline-none transition ${
                          errors[`${index}-phone`] ? 'border-red-400' : 'border-gray-200 focus:border-orange-400'
                        }`}
                        type="tel"
